...
params:
  fishery_name: "Satsop salmon 2024-25"
...
3.  Paste onto browser tab with open FishApps session (method will be standardized).
    ...
    -   If using event_id, navigate to any event by using the "Search" feature in FishApps. Open an event and then paste over the `event_id` after "<https://apps.wdfw-fish.us/creel/creel_event/>" in the URL so that the event_id copied from the QAQC document creates a new link. Press enter to navigate to the event in question using the authenticated FishApps session.
...
library(CreelEstimateR) #https://github.com/cbedwards-dfw/CreelEstimateR
...
dwg <- CreelEstimateR::fetch_dwg(params$fishery_name)
